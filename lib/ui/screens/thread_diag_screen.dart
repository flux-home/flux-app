...
class ThreadDiagScreen extends StatefulWidget {
  ...
  const ThreadDiagScreen({super.key, required this.device});
...
class _ThreadDiagScreenState extends State<ThreadDiagScreen> {
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          ...
          _error = 'Thread Network Diagnostics cluster not available on this device.\n\n'
                   'This cluster is only present on Matter devices that use Thread '
                   'as their network transport.';
        });
      } else {
        setState(() { _data = data; _state = _LoadState.done; _fetchedAt = DateTime.now(); });
      }
    } catch (e) {
      if (!mounted) return;
      setState(() { _state = _LoadState.done; _error = e.toString(); });
    }
  }
  ...
  void _copyText() {
    ...
    buf.writeln('Thread Network Diagnostics — ${widget.device.name}');
    buf.writeln('Read at: ${_fetchedAt?.toLocal()}');
    buf.writeln();
    buf.writeln('Network');
    buf.writeln('  Name:             ${d.networkName ?? '—'}');
    buf.writeln('  Channel:          ${d.channel ?? '—'}');
    buf.writeln('  PAN ID:           ${d.panId != null ? '0x${d.panId!.toRadixString(16).padLeft(4,'0').toUpperCase()}' : '—'}');
    buf.writeln('  Extended PAN ID:  ${d.extendedPanId ?? '—'}');
    buf.writeln('  Mesh prefix:      ${d.meshLocalPrefix ?? '—'}');
    buf.writeln();
    buf.writeln('Node Status');
    buf.writeln('  Routing role:     ${d.routingRoleLabel}');
    buf.writeln('  Partition ID:     ${d.partitionId ?? '—'}');
    buf.writeln('  Leader router:    ${d.leaderRouterId != null ? 'Router #${d.leaderRouterId}' : '—'}');
    buf.writeln('  Weighting:        ${d.weighting ?? '—'}');
    buf.writeln();
    buf.writeln('Neighbors (${d.neighbors.length})');
    for (final n in d.neighbors) {
      buf.writeln('  ${n.extAddress}  RLOC16:0x${n.rloc16.toRadixString(16).padLeft(4,'0').toUpperCase()}'
          '  LQI:${n.lqi}  RSSI:${n.averageRssi ?? '?'}dBm  ${n.isChild ? '[Child]' : '[Peer]'}  '
          '${n.fullThreadDevice ? '[FTD]' : '[MTD]'}  age:${n.age}s');
    }
    buf.writeln();
    buf.writeln('Routes (${d.routes.length})');
    for (final r in d.routes) {
      buf.writeln('  Router#${r.routerId}  RLOC16:0x${r.rloc16.toRadixString(16).padLeft(4,'0').toUpperCase()}'
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  ...
                  const Spacer(),
                  if (r.allocated)
                    _Badge('Allocated', cs.primaryContainer, cs.onPrimaryContainer),
                  if (r.linkEstablished) ...[
                    const SizedBox(width: 4),
                    _Badge('Linked', const Color(0xFF34A853).withAlpha(40),
                        const Color(0xFF34A853)),
                  ],
                ]),
                const SizedBox(height: 4),
                Row(children: [
                  Text('Next: $nextHopStr',
                      style: TextStyle(fontSize: 11, color: cs.onSurfaceVariant)),
                  const SizedBox(width: 12),
                  Text('Cost: ${r.pathCost}',
                      style: TextStyle(fontSize: 11, color: cs.onSurfaceVariant)),
                  const SizedBox(width: 12),
                  Text('LQI ↓${r.lqiIn} ↑${r.lqiOut}',
                      style: TextStyle(fontSize: 11, color: cs.onSurfaceVariant)),
                  const Spacer(),
                  Text('${r.age}s',
                      style: TextStyle(fontSize: 11, color: cs.onSurfaceVariant)),
