...
class _OnOffCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final litColor = isStale
        ? Colors.white24
        : isOn ? Colors.white : Colors.white38;
...
class _BrightnessTrackPainter extends CustomPainter {
  ...
  static const double _knobR  = 11.0;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(kp, _knobR, Paint()..color = color);
    canvas.drawCircle(kp, _knobR,
        Paint()
          ..color       = Colors.white.withAlpha(90)
          ..style       = PaintingStyle.stroke
          ..strokeWidth = 1.5);
    canvas.save();
    canvas.translate(kx, cy);
    final grip = Paint()
  ...
  @override
  bool shouldRepaint(_BrightnessTrackPainter o) =>
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          // ── Header ───────────────────────────────────────────────────────
          Row(children: [
            Icon(Icons.blinds_outlined, size: 18,
                color: isStale ? cs.onSurfaceVariant.withAlpha(80) : cs.onSurface),
            const SizedBox(width: 8),
            Text('Covering', style: Theme.of(context).textTheme.titleSmall
                ?.copyWith(fontWeight: FontWeight.w600)),
            const Spacer(),
            Text(isStale || openPct == null ? '--' : '$openPct% open',
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
            _CoveringBtn(icon: Icons.keyboard_arrow_up_rounded,   label: 'Open',
                onTap: isStale ? null : () => context.read<DeviceProvider>().coveringUp(view.id)),
            _CoveringBtn(icon: Icons.stop_rounded,                label: 'Stop',
                onTap: isStale ? null : () => context.read<DeviceProvider>().coveringStop(view.id)),
            _CoveringBtn(icon: Icons.keyboard_arrow_down_rounded, label: 'Close',
    ...
    return InkWell(
      ...
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Icon(icon, size: 24,
              color: onTap == null ? cs.onSurfaceVariant.withAlpha(80) : cs.onSurface),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          // ── Header ───────────────────────────────────────────────────────
          Row(children: [
            Icon(Icons.wind_power_outlined, size: 18,
                color: isStale ? cs.onSurfaceVariant.withAlpha(80) : cs.onSurface),
            const SizedBox(width: 8),
            Text('Fan', style: Theme.of(context).textTheme.titleSmall
                ?.copyWith(fontWeight: FontWeight.w600)),
            const Spacer(),
            Text(isStale || pct == null ? '--' : '$pct%',
                style: Theme.of(context).textTheme.bodySmall),
          ]),
          const SizedBox(height: 12),
          // ── Speed slider ──────────────────────────────────────────────────
          Slider(
            ...
            onChanged: isStale ? null : (_) {},
            onChangeEnd: isStale ? null : (v) =>
                context.read<DeviceProvider>().setFanPercent(view.id, v.round()),
          ),
          // ── Mode chips ────────────────────────────────────────────────────
          Wrap(spacing: 6, children: List.generate(_modes.length, (i) {
            final selected = mode == i;
            return ChoiceChip(
              label: Text(_modes[i]),
              selected: selected,
              onSelected: isStale ? null : (_) =>
                  context.read<DeviceProvider>().setFanMode(view.id, i),
            );
          })),
        ]),
      ),
...
class _ColorTemperatureCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final cs      = Theme.of(context).colorScheme;
    final isStale = view.isStale;
    final mireds  = view.colorTempMireds?.toDouble()
        .clamp(_minMireds, _maxMireds) ?? (_minMireds + _maxMireds) / 2;
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          Row(children: [
            ...
            const SizedBox(width: 8),
            Text('Color Temperature', style: Theme.of(context).textTheme.titleSmall
                ?.copyWith(fontWeight: FontWeight.w600)),
            const Spacer(),
            Text(isStale ? '--' : '${kelvin}K',
                style: Theme.of(context).textTheme.bodySmall),
          ]),
          ...
          Row(children: [
            ...
            Expanded(
              child: Slider(
                ...
                onChanged: isStale ? null : (_) {},
                onChangeEnd: isStale ? null : (v) =>
                    context.read<DeviceProvider>().setColorTemperature(view.id, v.round()),
              ),
            ),
...
class _SmokeAlarmCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Color stateColor() {
      ...
      return switch (worst) { 2 => Colors.red.shade500, 1 => Colors.orange.shade500, _ => Colors.green.shade500 };
    }
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          Row(children: [
            Icon(anyAlarm ? Icons.warning_rounded : Icons.check_circle_outline,
                size: 18, color: isStale ? cs.onSurfaceVariant.withAlpha(80) : stateColor()),
            const SizedBox(width: 8),
            Text('Smoke / CO Alarm', style: Theme.of(context).textTheme.titleSmall
                ?.copyWith(fontWeight: FontWeight.w600)),
          ]),
          const SizedBox(height: 12),
...
class _AlarmIndicator extends StatelessWidget {
  final String label;
  final int?   state; // 0=Normal 1=Warning 2=Critical null=unknown
  ...
  Widget build(BuildContext context) {
    final Color color;
    final String text;
    if (state == null)   { color = Colors.grey; text = '--'; }
    else if (state == 2) { color = Colors.red.shade500;    text = 'Critical'; }
    else if (state == 1) { color = Colors.orange.shade500; text = 'Warning';  }
    else                 { color = Colors.green.shade500;  text = 'OK';       }
