...
List<_CheckResult> _brChecks(BorderRouterDiagnostic br, String? savedExtPanId) {
  ...
  if (br.hasRoutableIpv6) {
    ...
    checks.add(_CheckResult(
      ...
      hint:     'The border router is visible via mDNS but only advertises '
    ...
    checks.add(_CheckResult(
      ...
      hint:     'The border router has no IPv6 address reachable from this '
  ...
  if (br.tcpReachable != null) {
    checks.add(br.tcpReachable!
        ? _CheckResult(
            ...
            subtitle: 'The app can open a direct connection to this border '
        ...
        : _CheckResult(
            ...
            subtitle: 'Found via network scan but a direct connection attempt '
                      'timed out.',
            hint:     'Your phone and this border router can see each other\'s '
  ...
  if (br.sameSubnetAsPhone != null) {
    checks.add(br.sameSubnetAsPhone!
        ? _CheckResult(
            ...
            subtitle: 'Phone and border router are on the same IPv4 subnet — '
                      'direct communication should work.',
          )
        : _CheckResult(
            status:   _Status.warning,
            title:    'Different network segments (IPv4)',
            subtitle: 'Phone and border router have IP addresses on different '
                      'subnets.',
            hint:     'Your phone and this border router are on different IP '
  ...
  if (br.ipv6PrefixMatchesPhone != null) {
    checks.add(br.ipv6PrefixMatchesPhone!
        ? _CheckResult(
            ...
            subtitle: 'Phone and border router share the same IPv6 /64 prefix '
                      '— Thread device addresses will be routable.',
          )
        : _CheckResult(
            status:   _Status.warning,
            title:    'Different IPv6 network segments',
            subtitle: 'Phone and border router have IPv6 addresses on '
                      'different /64 segments.',
            hint:     'Thread devices get their IPv6 addresses from the border '
  ...
  if (bm == null) {
    ...
    checks.add(switch (bm.threadInterfaceStatus) {
      2 => _CheckResult(
          status:   _Status.ok,
          title:    'Thread interface active',
      ...
      1 => _CheckResult(
          ...
          subtitle: bm.threadInterfaceLabel,
          hint:     'The border router has initialised its Thread interface '
      ...
      _ => _CheckResult(
          ...
          subtitle: bm.threadInterfaceLabel,
          hint:     'The border router has no active Thread interface. '
    ...
    checks.add(bm.hasExternalConnectivity
        ? _CheckResult(
            status:   _Status.ok,
        ...
        : const _CheckResult(
            ...
            hint:     'The border router reports no external IP connectivity. '
  ...
  if (savedExtPanId != null && savedExtPanId.isNotEmpty) {
    ...
    checks.add(match
        ...
        : _CheckResult(
            ...
            hint:     'The Thread dataset stored in the app belongs to a '
...
List<_DiagSection> _buildNonBrSections(
  NetworkDiagnosticsReport r,
  String? savedExtPanId,
) {
  ...
  if (ipv6.hasRoutableIpv6) {
    ...
    ipv6Check = _CheckResult(
      ...
      subtitle: 'Thread smart home devices communicate exclusively over IPv6 '
    ...
    ipv6Check = _CheckResult(
      ...
      subtitle: 'Your phone has an IPv6 address, but it only works within a '
      ...
      hint:     'What to do: Log in to your Wi-Fi router and look for an IPv6 '
    ...
    ipv6Check = const _CheckResult(
      ...
      subtitle: 'Thread smart home devices only communicate over IPv6 — a '
                'modern internet address format that your current Wi-Fi network '
                'is not providing to this phone.',
      hint:     'What to do: Log in to your Wi-Fi router settings and enable '
  ...
  if (wb.frequencyMhz > 0) {
    if (wb.is24Ghz) {
      ...
      phoneChecks.add(_CheckResult(
        ...
        subtitle: 'Some Thread Border Routers only support 2.4 GHz and may '
                  'not be visible on ${wb.band}.',
        hint:     'Your phone is connected to a ${wb.band} Wi-Fi network but '
      ...
      phoneChecks.add(_CheckResult(
        ...
        subtitle: 'Border router(s) are visible — your router bridges local '
  ...
  networkChecks.add(r.multicastLockAcquired
      ? const _CheckResult(
          ...
          subtitle: 'Your phone can receive local network announcements. '
      ...
      : const _CheckResult(
          ...
          subtitle: 'The app was unable to listen for local network '
                    'announcements on your current Wi-Fi connection.',
          hint:     'What this means: after a smart device joins Thread, the '
  ...
  if (r.vpn.isActive) {
    networkChecks.add(const _CheckResult(
      status:   _Status.warning,
      title:    'A VPN is active on this phone',
      subtitle: 'Some VPN configurations can interfere with smart home '
                'device setup.',
      hint:     'What this means: your phone is routing some or all network '
  ...
  if (r.matterTcpServices.isNotEmpty) {
    matterCheck = _CheckResult(
      status:   _Status.ok,
      title:    '${r.matterTcpServices.length} Matter device'
  ...
  sections.add(_DiagSection(
      heading: 'Commissioned Matter devices', checks: [matterCheck]));
...
class _NetworkCheckScreenState extends State<NetworkCheckScreen> {
  ...
  Future<void> _runCheck() async {
    ...
    setState(() { _report = report; _state = _ScreenState.done; });
  }
  ...
  void _copyReport() {
    ...
    void writeChecks(String heading, List<_CheckResult> checks) {
      ...
      for (final c in checks) {
        ...
        if (c.subtitle != null) { buf.writeln('  ${c.subtitle}'); }
        for (final b in c.bullets) { buf.writeln('  · $b'); }
        if (c.hint != null) { buf.writeln('  ℹ ${c.hint}'); }
      }
      buf.writeln();
    }
    ...
    // Matter section last
    final matterSection = _buildNonBrSections(r, _savedExtPanId)
      ...
      child: Column(
        ...
        children: [
          Icon(Icons.network_check_outlined,
              size: 64, color: cs.primary.withAlpha(160)),
          const SizedBox(height: 20),
          Text('Diagnose your Thread & IPv6 network',
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text('Discovering border routers & Matter devices',
              style: Theme.of(context)
                  .textTheme
                  .bodySmall
                  ?.copyWith(color: cs.onSurfaceVariant)),
        ...
        if (r.borderRouters.isEmpty)
          _DiagCard(checks: [
            const _CheckResult(
              ...
              hint:     'Ensure your Thread Border Router is powered on and '
                  ...
                  children: [
                    _BrSummaryTile(
                      ...
                      onTap: () => Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (_) => _BorderRouterDetailScreen(
                            br:            br,
                            savedExtPanId: _savedExtPanId,
                          ),
                        ),
...
class _BrSummaryTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final statusSummary = switch (worst) {
      ...
      _Status.fail    => '$failCount issue${failCount == 1 ? '' : 's'}'
      ...
      subtitle: Column(
        ...
        children: [
          ...
          Text(addrSummary,
              style: TextStyle(
                  fontSize: 11,
                  color: br.hasRoutableIpv6 ? cs.onSurfaceVariant : color)),
        ],
      ),
      trailing: Row(
        mainAxisSize: MainAxisSize.min,
...
class _BorderRouterDetailScreen extends StatelessWidget {
  ...
  const _BorderRouterDetailScreen({
    required this.br,
    required this.savedExtPanId,
  });
...
class _SummaryBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final label = switch (overall) {
      _Status.ok      => 'All checks passed',
      _Status.warning => '$warnCount warning${warnCount == 1 ? '' : 's'}',
      _Status.fail    => '$failCount issue${failCount == 1 ? '' : 's'} found'
                         '${warnCount > 0 ? '  ·  $warnCount warning${warnCount == 1 ? '' : 's'}' : ''}',
    };
